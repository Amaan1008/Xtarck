const API = '../api/admin';
const PG = 15;
let users = [], trips = [], reviews = [], notices = [], locations = [], hotels = [];
let uPg = 1, tPg = 1, rPg = 1;
// ── BOOT ──────────────────────────────────────
window.addEventListener('DOMContentLoaded', async () => {
    const ok = await checkAuth();
    if (!ok)
        return; // stop — redirect is already in progress
...
async function loadStats() {
    try {
        ...
        if (d.recent_locs && $('recent-locs-list')) {
            $('recent-locs-list').innerHTML = (d.recent_locs || []).map(l => `<div class="qii"><div><div class="qil">${e(l.emoji || '📍')} ${e(l.name)}</div><div class="qis">${e(l.region)}</div></div></div>`).join('') || '<div style="font-size:12px;color:var(--text3);padding:8px">No locations yet.</div>';
        }
...
async function loadUsers() {
    try {
        ...
        if (!d.success) {
            console.error('loadUsers: API failure –', d.message);
            return;
        }
        users = d.users;
      ...
      <tr>
        ...
        <td style="font-size:11px;color:var(--text3)">${dt(u.created_at)}</td>
            ...
            console.error('loadUsers ovTable failed:', err2);
        }
    }
    catch (err) {
...
async function loadNotices() {
    try {
        ...
        console.error('loadNotices failed:', err);
    }
}
// ── RENDERS ────────────────────────────────────
...
function renderNotices() {
    const el = $('ann-list');
    if (!notices.length) {
        el.innerHTML = '<div class="empty"><p>No announcements yet. Post one above.</p></div>';
        return;
    }
...
async function postAnnouncement() {
    ...
    if (!title || !msg) {
        toast('Fill in title and message.', 'err');
        return;
    }
    ...
    try {
        ...
        btn.disabled = false;
        btn.textContent = 'Post to Dashboard';
    }
}
...
async function banUser(id, name) {
    if (!confirm(`Ban "${name}"?\n\nThey will be blocked from logging into the customer site.`))
        return;
    try {
...
async function delUser(id, name) {
    if (!confirm(`Delete "${name}" permanently?\n\nAll their trips and reviews will also be deleted.`))
        return;
    try {
...
async function delRev(id, place) {
    if (!confirm(`Delete review for "${place}"?`))
        return;
    try {
...
function openUserModal(id) {
    ...
    set('m-u-name', u.fullname);
    set('m-u-email', u.email);
    $('m-u-status').innerHTML = statusBadge(u.status);
    set('m-u-phone', u.phone || '—');
    set('m-u-joined', dt(u.created_at));
    set('m-u-trips', u.trip_count + ' trip' + (u.trip_count !== 1 ? 's' : ''));
    set('m-u-revs', u.review_count + ' review' + (u.review_count !== 1 ? 's' : ''));
...
function openRevModal(id) {
    ...
    if (!r)
        return;
    set('m-r-place', r.place_name);
    set('m-r-user', r.user_fullname);
...
function mkpag(eid, total, cur, cb) {
    ...
    for (let p = 1; p <= pages; p++) {
        if (pages > 7 && Math.abs(p - cur) > 2 && p !== 1 && p !== pages) {
            if (p === 2 || p === pages - 1)
                h += `<span style="padding:0 4px;color:var(--text3)">…</span>`;
            continue;
        }
...
function set(id, v) { const el = $(id); if (el)
...
async function saveLocation() {
    ...
    try {
        ...
        btn.disabled = false;
        btn.textContent = id ? 'Update Location' : 'Save Location';
    }
}
async function delLocation(id, name) {
    if (!confirm('Delete location "' + name + '"?\n\nThis will also delete all its places and hotels.'))
        return;
    try {
...
async function loadHotels() {
    try {
        ...
        console.warn('loadHotels failed', e);
    }
}
function renderHotelsList() {
    ...
    const flat = [];
    Object.entries(hotels).forEach(([locName, arr]) => {
        (arr || []).forEach(h => flat.push({ ...h, location_name: locName }));
    });
    const f = flat.filter(h => (!loc || h.location_name === loc) &&
        (!tier || h.tier === tier) &&
...
async function saveHotel() {
    ...
    try {
        ...
        btn.disabled = false;
        btn.textContent = id ? 'Update Hotel' : 'Save Hotel';
    }
}
...
function applyCustomRange() {
    ...
    if (f > t) {
        toast('From must be before To.', 'err');
        return;
    }
    loadAnalytics(f, t);
}
            ...
            datasets: [
                {
                    label: 'Reviews',
                    data: counts,
                    backgroundColor: labels.map((_, i) => `hsla(${170 - i * 10},60%,${45 + i * 2}%,0.8)`),
                    borderRadius: 5,
                    borderSkipped: false,
                },
            ],
...
out tags;`;
        ...
        if (sorted.length < 3)
            throw new Error('Insufficient city-tagged data');
...
async function downloadReport() {
    if (!_anData) {
        toast('Load analytics first.', 'err');
        return;
    }
    ...
    if (d.top_places && d.top_places.length) {
        ...
        doc.setFontSize(13);
        doc.setTextColor(30, 30, 30);
        doc.text('Most Crowded Places', 14, y);
        y += 2;
        doc.autoTable({
            startY: y,
    ...
    if (visitedData.length) {
        ...
        doc.setFontSize(13);
        doc.setTextColor(30, 30, 30);
        doc.text('Most Visited Places in Sri Lanka', 14, y);
        y += 2;
        doc.autoTable({
            startY: y,
