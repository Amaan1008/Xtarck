...
function starsStr(n) {
    let s = '';
    for (let i = 1; i <= 5; i++)
        s += `<span style="color:${i <= n ? '#f5a623' : '#ddd'};font-size:12px">★</span>`;
    return s;
}
...
function starsHtml(r) {
    let s = '';
    for (let i = 1; i <= 5; i++)
        s += `<span style="color:${i <= r ? '#f5a623' : 'var(--border)'};font-size:13px">★</span>`;
    return s;
}
...
function renderReviewChart(reviews) {
    ...
    if (!ctx)
        return;
    if (reviewChartInst) {
        reviewChartInst.destroy();
        reviewChartInst = null;
    }
    const counts = [0, 0, 0, 0, 0];
    reviews.forEach(r => { const s = parseInt(r.rating || r.stars || 0); if (s >= 1 && s <= 5)
        counts[5 - s]++; });
    ...
    if (total === 0) {
        ctx.parentElement.innerHTML = emptyState('⭐', 'No reviews yet', 'Rate places after your trips', '', '');
        return;
    }
...
function populateDashboard(u, trips, reviews) {
    ...
    ['pstat-trips', 'pstat-places', 'pstat-reviews'].forEach((id, i) => {
        const el = $(id);
        if (el)
            el.textContent = [tc, pc, rc][i];
    });
    const sub = $('trips-subtitle');
    if (sub)
        sub.textContent = tc === 0 ? 'No saved trips yet' : tc + ' saved planning session' + (tc !== 1 ? 's' : '');
    // ── Previous Plans grid ──
...
async function submitPremPayment() {
    ...
    if (!num || !name || !exp || !cvv) {
        showErr('Please fill in all card details.');
        return;
    }
    if (num.replace(/\D/g, '').length < 13) {
        showErr('Please enter a valid card number.');
        return;
    }
    if (!/^\d{2}\/\d{2}$/.test(exp)) {
        showErr('Expiry must be in MM/YY format.');
        return;
    }
    if (cvv.length < 3) {
        showErr('CVV must be 3 or 4 digits.');
        return;
    }
