...
$('signupForm').addEventListener('submit', async (e) => {
    ...
    if (!name) {
        showErr('fullname', 'name-error', true);
        valid = false;
    }
    else
        showErr('fullname', 'name-error', false);
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        showErr('email', 'email-error', true);
        valid = false;
    }
    else
        showErr('email', 'email-error', false);
    if (pw.length < 8 || !/\d/.test(pw)) {
        valid = false;
    }
    if (pw !== cf) {
        showErr('confirm', 'confirm-error', true);
        valid = false;
    }
    else
        showErr('confirm', 'confirm-error', false);
    if (!valid)
        return;
    // ── Call PHP backend ──────────────────────────
