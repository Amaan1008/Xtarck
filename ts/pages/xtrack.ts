...
let NP: Record<string, {n:string,d:string,desc:string,cost:number,hrs:number}[]> = {
...
const S: any={cat:null,method:null,
  ...
  selA:new Set<string>(),selB:new Set<string>(),chA:null,chB:null
};

/* ── UTILS ── */
function lkr(n){return n>=100000?'LKR '+(n/1000).toFixed(1)+'k':'LKR '+Math.round(n).toLocaleString()}
function fdate(ds,off=0){if(!ds)return'TBD';const d=new Date(ds);d.setDate(d.getDate()+off);return d.toLocaleDateString('en-GB',{weekday:'short',month:'short',day:'numeric'})}
...
function pickCat(id){
  ...
  S.A.dest=dests[0]; S.B.dest=dests[0];
...
function chooseMethod(m){
  S.method=m;S.selA=new Set();S.selB=new Set();
  if(m==='budget'){buildWzA();goTo('page3a');}
  else{buildWzB();goTo('page3b');}
}

/* ── WIZARD ENGINE ── */
let bStep=0,dStep=0;
function mkProg(cid,tot,cur,lbls){
  let h='';for(let i=0;i<tot;i++){
    if(i>0)h+=`<div class="wz-line ${i<=cur?'done':''}"></div>`;
...
const BST=[
  {t:'What is your total budget?',h:'Enter the maximum amount you would like to spend in Sri Lankan Rupees.',
   ...
   sv:()=>{const v=parseInt($('fb').value);if(isNaN(v)||v<5000){showToast('Minimum budget: LKR 5,000');return false;}S.A.budget=v;return true;}},
  {t:'Where would you like to go?',h:'Choose your destination — filtered by your chosen trip type.',
   r:()=>{const dests=getDestsForCat(S.cat);if(!dests.includes(S.A.dest))S.A.dest=dests[0];return`<select class="field" id="fa">${dests.map(d=>`<option ${d===S.A.dest?'selected':''}>${d}</option>`).join('')}</select>`;},
   ...
   ar:()=>{
     setTimeout(()=>{
       ...
       if(wrap&&!$('geoBtn')){
         ...
         btn.id='geoBtn';btn.className='geo-btn';
  ...
  {t:'When does your trip begin?',h:'Choose your preferred start date for the journey.',
   ...
   ar:()=>{const e=$('fs');const u=()=>{const d=new Date(e.value||new Date());d.setDate(d.getDate()+S.A.days-1);const el=$('endA');if(el)el.textContent=d.toLocaleDateString('en-GB',{weekday:'short',month:'short',day:'numeric'});};e.addEventListener('change',u);u();},
   sv:()=>{S.A.date=$('fs').value;return true;}}
];

function setSt(w,id,c){S[w].style=id;c.querySelectorAll('.style-opt').forEach(e=>e.classList.remove('sel'));(event.currentTarget as HTMLElement).classList.add('sel')}
function setField(w,f,v,c){S[w][f]=v;c.querySelectorAll('.style-opt').forEach(e=>e.classList.remove('sel'));(event.currentTarget as HTMLElement).classList.add('sel')}
function toggleInterest(w,id,el){if(!S[w].interests)S[w].interests=[];const i=S[w].interests.indexOf(id);if(i>-1)S[w].interests.splice(i,1);else S[w].interests.push(id);el.classList.toggle('sel',S[w].interests.includes(id));}
function sSync(){const v=$('fb').value;const el=$('rv');if(el)el.textContent=lkr(parseInt(v)||0);const s=$('rs');if(s){s.value=v;s.style.setProperty('--v',((v-5000)/295000*100)+'%');}}
function rSync(v){const el=$('fb');if(el)el.value=v;const lv=$('rv');if(lv)lv.textContent=lkr(parseInt(v));const s=$('rs');if(s)s.style.setProperty('--v',((v-5000)/295000*100)+'%');}

let wzDir=1;
function buildWzA(){bStep=0;wzDir=1;renderWzA()}
function renderWzA(){
  const s=BST[bStep];const n=bStep+1;
  ...
  if(s.ar) setTimeout(s.ar,50);
}
function nextA(){if(!BST[bStep].sv())return;if(bStep<BST.length-1){wzDir=1;bStep++;renderWzA();}else{buildDA();goTo('page4a');}}
function prevA(){if(bStep>0){wzDir=-1;bStep--;renderWzA();}}

/* WIZARD STEPS — Destination */
const DST=[
  {t:'Where are you heading?',h:'Choose your destination — filtered by your chosen trip type.',
   r:()=>{const dests=getDestsForCat(S.cat);if(!dests.includes(S.B.dest))S.B.dest=dests[0];return`<select class="field" id="fb2">${dests.map(d=>`<option ${d===S.B.dest?'selected':''}>${d}</option>`).join('')}</select>`;},
  ...
  {t:'Where are you currently located?',h:'Select your starting point — we\'ll show attractions along the route to your destination.',
   ...
   ar:()=>{
     setTimeout(()=>{
       ...
       if(wrap&&!$('geoBtnB')){
         const btn=document.createElement('button');
         btn.id='geoBtnB';btn.className='geo-btn';
         btn.innerHTML='<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="3" stroke="currentColor" stroke-width="1.3"/><path d="M6.5 1v2M6.5 10v2M1 6.5h2M10 6.5h2" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg> Use My Location';
  ...
  {t:'When does your trip begin?',h:'Choose your preferred start date.',
   ...
   ar:()=>{const e=$('fs2');const u=()=>{const d=new Date(e.value||new Date());d.setDate(d.getDate()+S.B.days-1);const el=$('endB');if(el)el.textContent=d.toLocaleDateString('en-GB',{weekday:'short',month:'short',day:'numeric'});};e.addEventListener('change',u);u();},
   sv:()=>{S.B.date=$('fs2').value;return true;}}
];
function buildWzB(){dStep=0;wzDir=1;renderWzB()}
function renderWzB(){
  const s=DST[dStep];const n=dStep+1;
...
function nextB2(){if(!DST[dStep].sv())return;if(dStep<DST.length-1){wzDir=1;dStep++;renderWzB();}else{buildDB();goTo('page4b');}}
function prevB2(){if(dStep>0){wzDir=-1;dStep--;renderWzB();}}

/* ── CALC ── */
function calc(dest,days,people,style,sel,trans='taxi'){
  const r=SR[style];
  let tMult=1;
  if(trans==='public') tMult=0.15;
  if(trans==='self') tMult=0.45;
  const c: CostBreakdown = {t:Math.round(r.t*days*people*tMult),a:Math.round(r.a*days*people),f:Math.round(r.f*days*people),ac:Math.round(r.ac*days*people),total:0};
  sel.forEach(n=>{const p=(NP[dest]||NP.Ella).find(x=>x.n===n);if(p)c.ac+=p.cost*people;});
  c.total=c.t+c.a+c.f+c.ac;return c;
}
function genSched(days,dest,date,sel,pace='balanced'){
  const pl=[...sel];const out=[];
  let accAct = 0;
  for(let i=0;i<days;i++){
    let type,title,sub;
    if(i===0){type='travel';title=`Travel to ${dest}`;sub='Depart from starting point · Check‑in · Settle in';}
    else if(i===days-1&&days>1){type='travel';title='Return home';sub='Check‑out · Travel back';}
    else{
      ...
      if(dayPl.length > 0) {
          ...
          type='rest';title='Rest & Leisure';sub='Local cuisine · Free time · Relaxation';
      } else {
          type='activity';title=`Explore ${dest}`;sub='Local sights · Culture · Cuisine';
      }
    }
...
function genDaily(dest,days,people,style,date,trans='taxi'){
  const r=SR[style];const d=km(dest);const out=[];
  let tMult=1;
  if(trans==='public') tMult=0.15;
  if(trans==='self') tMult=0.45;
  ...
  for(let i=0;i<days;i++){
    ...
    if(i===0){dkm=d;hrs=Math.max(1,Math.round(d/spd));tr=Math.round(r.t*people*1.5*tMult);ac=Math.round(r.a*people);fo=Math.round(r.f*people*.8);act=0;}
    else if(i===days-1&&days>1){dkm=d;hrs=Math.max(1,Math.round(d/spd));tr=Math.round(r.t*people*1.2*tMult);ac=0;fo=Math.round(r.f*people*.6);act=0;}
    else{dkm=Math.round(12+Math.random()*22);hrs=1+Math.round(Math.random()*1);tr=Math.round(r.t*people*.3*tMult);ac=Math.round(r.a*people);fo=Math.round(r.f*people);act=Math.round(r.ac*people);}
...
function mkChart(cid,c,ref){
  const ctx=$(cid);if(!ctx)return null;
  if(ref)ref.destroy();
...
function mkDbProg(cid){
  ...
  let h='';for(let i=0;i<4;i++){
    if(i>0)h+=`<div class="dbp-line done" style="margin:0 4px"></div>`;
...
function ensureSpin(){
  if(!$('xtrSpinKf')){
    const s=document.createElement('style');s.id='xtrSpinKf';
...
function lfClear(panel){
  if(!LF_INST[panel]) return;
  LF_LAYERS[panel].forEach(l=>{ try{LF_INST[panel].removeLayer(l);}catch(e){} });
  LF_LAYERS[panel]=[];
}
...
async function renderMap(panel){
  ...
  lfAddMarker(panel, map, fromCoord[0], fromCoord[1],
    lfPin('#2aab99','S','circle'),
    `<div style="font-weight:800;font-size:13px;color:#1a1f18;margin-bottom:4px">Starting Point</div>
     <div style="font-size:12px;color:#4a5248">${s.current}</div>`);

  places.forEach((p,i)=>{
    lfAddMarker(panel, map, p.coord[0], p.coord[1],
      lfPin('#3b82f6',String(i+1),'pin'),
      `<div style="font-weight:800;font-size:13px;color:#1a1f18;margin-bottom:4px">Stop ${i+1}</div>
       <div style="font-size:13px;font-weight:600;color:#1a1f18;margin-bottom:2px">${p.name}</div>`);
  });

  lfAddMarker(panel, map, destCoord[0], destCoord[1],
    lfPin('#dc7c32','D','circle'),
    `<div style="font-weight:800;font-size:13px;color:#1a1f18;margin-bottom:4px">Destination</div>
     <div style="font-size:12px;color:#4a5248">${s.dest}</div>`);

  if(hotel){
    lfAddMarker(panel, map, destCoord[0]+0.006, destCoord[1]-0.006,
      lfPin('#7c3aed','H','pin'),
      `<div style="font-weight:800;font-size:13px;color:#1a1f18;margin-bottom:5px">${hotel.emoji} ${hotel.name}</div>
  ...
  try{
    ...
    if(!data.routes || !data.routes[0]) throw new Error('No route found between these locations');
...
var switchA = function(t,el){
  ...
  if(t==='weather') renderWeather('weatherA',S.A.dest,S.A.days,S.A.date,S.selA).catch(()=>{});
  if(t==='map'){
    setTimeout(()=>{
      renderMap('A');
      if(LF_INST.A) LF_INST.A.invalidateSize();
    },120);
  }
}
var switchB = function(t,el){
  ...
  if(t==='weather') renderWeather('weatherB',S.B.dest,S.B.days,S.B.date,S.selB).catch(()=>{});
  if(t==='map'){
    setTimeout(()=>{
      renderMap('B');
      if(LF_INST.B) LF_INST.B.invalidateSize();
    },120);
  }
}
...
async function fetchOWMWeather(dest){
  const now=Date.now();
  if(_weatherCache[dest]&&(now-_weatherCache[dest].ts)<_CACHE_TTL) return _weatherCache[dest];
  ...
  try{
    ...
    return cached;
  }catch(e){ console.warn('OWM fetch failed:',dest,e.message); return null; }
}
...
function getDayBadge(icon,hi){
  if(icon==='rain') return {cls:'r',txt:'Rain'};
  if(icon==='cloudy') return {cls:'g',txt:'Good'};
  if(hi>=34) return {cls:'c',txt:'Hot'};
...
function rainChance(icon,hi){
  if(icon==='rain') return hi<=20?'75%':'65%';
  if(icon==='cloudy') return '28%';
  if(hi>=34) return '8%';
  return '12%';
}
...
function bestVisitTime(icon,hi){
  if(icon==='rain') return 'Best: 6–10am (drier mornings)';
  if(hi>=34) return 'Best: Before 10am or after 4pm';
  if(icon==='cloudy') return 'Best: All day — comfortable';
...
function getNotToDo(icon,hi){
  if(icon==='rain'&&hi<=20) return NOT_TO_DO.rain_cold;
  if(icon==='rain') return NOT_TO_DO.rain_warm;
  if(icon==='cloudy') return NOT_TO_DO.cloudy;
  if(hi>=34) return NOT_TO_DO.hot;
...
function weatherScore(w){
  ...
  if(w.hi>=34) s-=1;
  if(w.hi<18) s-=1;
  if(w.icon==='sun'&&w.hi>=24&&w.hi<34) s+=1;
...
async function renderWeather(elId, dest, days, date, selSet){
  ...
  try{
    ...
    if(selSet&&selSet.size>0) await Promise.allSettled([...selSet].map(n=>fetchOWMWeather(n)));
  ...
  if(selArr.length>1 && count>0){
    selArr.forEach(n=>{
      ...
      if(pw.icon==='rain') conflicts.push(n);
    });
  }
...
function updatePdBtn(name, suffix, toggleFn){
  ...
  const card = $(safeId);
  if(card){
    if(isSelected) card.classList.add('sel'); else card.classList.remove('sel');
    const addBtn = card.querySelector('.pc-btn-add');
    if(addBtn) addBtn.innerHTML = isSelected
      ? `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg> Added`
      : `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M6 1v10M1 6h10" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg> Add to Plan`;
  }
}
...
function buildDA(){
  ...
  updA();renderPlA();renderSchA();
  /* init adjust panel */
  const da=$('adjDA');if(da)da.textContent=days;
  const pa=$('adjPA');if(pa)pa.textContent=people;
  const dateAEl=$('adjDateA');
  if(dateAEl){dateAEl.value=date||new Date().toISOString().split('T')[0];initAdjDate('A');}
...
function updA(){
  ...
  const w=$('warnA');
  if(c.total>budget){
    w.classList.remove('hidden');w.className='db-warn danger';
    ...
    w.classList.remove('hidden');w.className='db-warn';
  ...
  const dl2=$('dailyA2');if(dl2)dl2.innerHTML=dlH;
  /* sync adjust panel display values */
  const adjDAEl=$('adjDA');if(adjDAEl)adjDAEl.textContent=days;
  const adjPAEl=$('adjPA');if(adjPAEl)adjPAEl.textContent=people;
}
...
function getPlaceRec(w){
  const hi = w.hi, icon = w.icon;
  if(icon === 'rain'){
    if(hi <= 20) return {level:'caution', title:'Weather may be challenging',
  ...
  // sun / clear
  if(hi >= 33) return {level:'caution', title:'Excellent — but stay cool',
...
function hidePlaceWeather(suffix){
  const p = $('pwp'+suffix);
  if(p){ p.className='place-weather-panel'; p.innerHTML=''; }
}

function togPA(n){
  const wasSelected = S.selA.has(n);
  if(wasSelected)S.selA.delete(n);else S.selA.add(n);
  const safeId='pa_'+n.replace(/\W/g,'_');
  const card = $(safeId);
  if(card){
    if(S.selA.has(n)) card.classList.add('sel'); else card.classList.remove('sel');
    const addBtn = card.querySelector('.pc-btn-add');
    if(addBtn) addBtn.innerHTML = S.selA.has(n)
      ? `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg> Added`
      : `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M6 1v10M1 6h10" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg> Add to Plan`;
  }
  updA();renderSchA();
  if(!wasSelected){ showPlaceWeather(n, S.A.dest, 'A'); }
  else { hidePlaceWeather('A'); }
  showToast(S.selA.has(n)?n+' added to plan':n+' removed');
  const wEl=$('weatherA');
  if(wEl&&wEl.closest('.db-tc')&&wEl.closest('.db-tc').classList.contains('active')) renderWeather('weatherA',S.A.dest,S.A.days,S.A.date,S.selA);
  const mapTcA=$('tcA-map');
  if(mapTcA&&mapTcA.classList.contains('active')) renderMap('A');
}
...
function genDetailedSched(days, dest, date, sel, pace, current) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    if (isFirst) {
      // ── TRAVEL DAY ──
      dayType = 'travel'; dayLabel = 'Travel Day';
      startTime = '07:00'; endTime = '22:00'; totalKm = 180; totalHrs = '5-6';
      let t = '07:00';
      activities = [
      ...
      // ── RETURN DAY ──
      dayType = 'return'; dayLabel = 'Return Day';
      startTime = '07:00'; endTime = '20:00'; totalKm = 180; totalHrs = '5-6';
      let t = '07:00';
      activities = [
      ...
      let dayPlaces = [];
      for (let j = 0; j < placesPerDay; j++) {
        if (placeList[placeIdx]) { dayPlaces.push(placeList[placeIdx]); placeIdx++; }
      }
      if (dayPlaces.length > 0) {
        dayType = 'activity'; dayLabel = 'Activity Day';
      } else {
        dayType = 'activity'; dayLabel = 'Leisure Day';
      }
      startTime = '06:30'; endTime = '22:00'; totalKm = Math.round(15+Math.random()*25); totalHrs = '8-9';
...
function renderSchA(){
  if(!S.A.sched) S.A.sched = genDetailedSched(S.A.days, S.A.dest, S.A.date, [...S.selA], S.A.pace, S.A.current);
...
function buildDB(){
  ...
  updB();renderPlB();renderSchB();
  /* init adjust panel */
  const dateBEl=$('adjDateB');
  if(dateBEl){dateBEl.value=date||new Date().toISOString().split('T')[0];initAdjDate('B');}
...
function togPB(n){
  ...
  if(wasSelected)S.selB.delete(n);else S.selB.add(n);
  const safeId='pb_'+n.replace(/\W/g,'_');
  const cardB = $(safeId);
  if(cardB){
    if(S.selB.has(n)) cardB.classList.add('sel'); else cardB.classList.remove('sel');
    const addBtnB = cardB.querySelector('.pc-btn-add');
    if(addBtnB) addBtnB.innerHTML = S.selB.has(n)
      ? `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg> Added`
      : `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M6 1v10M1 6h10" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg> Add to Plan`;
  }
  updB();renderSchB();
  if(!wasSelected){ showPlaceWeather(n, S.B.dest, 'B'); }
  else { hidePlaceWeather('B'); }
  showToast(S.selB.has(n)?n+' added to plan':n+' removed');
  const wEl=$('weatherB');
  if(wEl&&wEl.closest('.db-tc')&&wEl.closest('.db-tc').classList.contains('active')) renderWeather('weatherB',S.B.dest,S.B.days,S.B.date,S.selB);
  const mapTcB=$('tcB-map');
  if(mapTcB&&mapTcB.classList.contains('active')) renderMap('B');
}
function renderSchB(){
  if(!S.B.sched) S.B.sched = genDetailedSched(S.B.days, S.B.dest, S.B.date, [...S.selB], S.B.pace, S.B.current);
  const el = $('schedB');
...
function styleTier(style){
  if(style==='simple') return 'budget';
  if(style==='flexible') return 'luxury';
  return 'comfortable';
}
...
function mkStars(n){
  return Array.from({length:5},(_,i)=>
    `<span class="hotel-star">${i<n?'★':'☆'}</span>`
  ).join('');
}
...
function renderHotels(containerId, dest, style, days, date){
  ...
  if(date){
    cin=date;
    var dd=new Date(date);dd.setDate(dd.getDate()+(days||3));
...
function openBkModal(hotel, cin, cout, dest){
  ...
  cinEl.min=today;
  if(cout){coutEl.value=cout;}else{
    const d=new Date(cin||today);d.setDate(d.getDate()+2);
...
function closeBkModal(e){if(e.target===$('bookingModal'))closeBkModalDirect();}
...
function bkCalcTotal(){
  ...
  if(!cin||!cout){summary.style.display='none';return;}
...
function confirmBooking(){
  ...
  const d1=new Date(cin),d2=new Date(cout);
  if(d2<=d1){showToast('Check-out must be after check-in');return;}
  const nights=Math.round((d2.getTime()-d1.getTime())/86400000);
...
switchA=function(t,el){
  _origSwitchA(t,el);
  if(t==='hotel') renderHotels('hotelListA',S.A.dest,S.A.style,S.A.days,S.A.date);
};
const _origSwitchB=switchB;
switchB=function(t,el){
  _origSwitchB(t,el);
  if(t==='hotel') renderHotels('hotelListB',S.B.dest,S.B.style,S.B.days,S.B.date);
};
function syncAdjBtns(containerId, activeVal){
  ...
  c.querySelectorAll('.adj-opt-btn').forEach(btn=>{
    ...
    if(match)btn.classList.toggle('sel',match[1]===activeVal);
  });
}
...
function adjA(field,delta){
  if(field==='days')S.A.days=Math.max(1,Math.min(21,S.A.days+delta));
  else S.A.people=Math.max(1,Math.min(20,S.A.people+delta));
  S.A.sched=null; updA();renderSchA();
  initAdjDate('A');
  showToast(field==='days'?`${S.A.days} ${S.A.days>1?'days':'day'}`:`${S.A.people} ${S.A.people>1?'people':'person'}`);
}
function adjADate(val){
  S.A.date=val; S.A.sched=null; updA();renderSchA();initAdjDate('A');
  showToast('Start date updated');
}
function adjAStyle(val,container){
  S.A.style=val; S.A.sched=null;
  container.querySelectorAll('.adj-opt-btn').forEach(b=>b.classList.remove('sel'));
  (event.currentTarget as HTMLElement).classList.add('sel');
  updA();renderSchA();
...
function adjBDate(val){
  S.B.date=val; S.B.sched=null; updB();renderSchB();initAdjDate('B');
  showToast('Start date updated');
}
function adjBStyle(val,container){
  S.B.style=val; S.B.sched=null;
  container.querySelectorAll('.adj-opt-btn').forEach(b=>b.classList.remove('sel'));
  (event.currentTarget as HTMLElement).classList.add('sel');
  updB();renderSchB();
...
function adjB(field,delta){
  if(field==='days')S.B.days=Math.max(1,Math.min(21,S.B.days+delta));
  else S.B.people=Math.max(1,Math.min(20,S.B.people+delta));
  S.B.sched=null; updB();renderSchB();initAdjDate('B');
...
function applyTheme(mode) {
  ...
  if (fabIcon)  fabIcon.textContent  = isDarkNow ? '☀️' : '🌙';
  if (fabLabel) fabLabel.textContent = isDarkNow ? 'Light Mode' : 'Dark Mode';
  ...
  if (dmToggle) dmToggle.checked = (mode === 'dark');
  if (lmToggle) lmToggle.checked = (mode === 'light');
  if (dmSub)    dmSub.textContent = mode === 'dark' ? 'On — using dark theme' : 'Off — using light theme';
  if (dmIcon)   dmIcon.textContent = isDarkNow ? '🌙' : '☀️';
...
function applyThemeFromToggle(checked) {
  ...
  const lmToggle = $('lightModeToggle');
  if (lmToggle) lmToggle.checked = !checked;
...
function applyLightFromToggle(checked) {
  ...
  const dmToggle = $('darkModeToggle');
  if (dmToggle) dmToggle.checked = !checked;
...
async function bootstrapData() {
  try {
    ...
    if (locRes.success && locRes.locations.length) {
      ...
      locs.forEach(l => {
        (l.places || []).forEach(p => {
          if (p.lat && p.lon) GEO[p.n] = [p.lat, p.lon];
        });
      });
      ...
      const npNew: Record<string, {n:string,d:string,desc:string,cost:number,hrs:number}[]> = {};
      locs.forEach(l => {
    ...
    if (typeof populateDestSelects === 'function') populateDestSelects();
    if (typeof refreshCategoryDestinations === 'function') refreshCategoryDestinations();
...
function geolocate(isB=false){
  ...
  const btn = $(btnId);
  if(btn){ btn.classList.add('loading'); btn.textContent='Locating…'; }
  navigator.geolocation.getCurrentPosition(pos=>{
    ...
    if(sel){
      for(let i=0;i<sel.options.length;i++){
        if(sel.options[i].value===nearest){ sel.selectedIndex=i; break; }
      }
      if(isB) S.B.current=nearest; else S.A.current=nearest;
    }
    if(btn){ btn.classList.remove('loading'); btn.innerHTML='<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="3" stroke="currentColor" stroke-width="1.3"/><path d="M6.5 1v2M6.5 10v2M1 6.5h2M10 6.5h2" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg> ✓ '+nearest; }
    showToast('📍 Location set to '+nearest);
  }, err=>{
    if(btn){ btn.classList.remove('loading'); btn.innerHTML='<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="3" stroke="currentColor" stroke-width="1.3"/><path d="M6.5 1v2M6.5 10v2M1 6.5h2M10 6.5h2" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg> Use My Location'; }
...
function checkBudgetConflict(){
  if(!S.A.budget || !S.A.days || !S.A.people || !S.A.style) return;
  ...
  if(S.A.budget < minTotal){
    ...
    if(!existing){
      ...
      warn.id='wzBudgetWarn';warn.className='wz-budget-warn';
      ...
      if(card) card.appendChild(warn);
    } else { existing.querySelector('strong').nextSibling.textContent=' '+msg; }
...
function renderFXStrip(panel, totalLKR){
  ...
  if(strip) strip.querySelector('.fx-label').textContent = `💱 ${lkr(amt)} equals:`;
}
  ...
  try{
    ...
    if(trip){
      ...
      if(S.method==='budget'){
        ...
        buildDA(); setTimeout(()=>goTo('page4a'),300);
      } else {
        ...
        buildDB(); setTimeout(()=>goTo('page4b'),300);
      }
    }
  }catch(e){}
})();
...
function exportPDF(panel){
  ...
  script.onload = ()=>{
    ...
    const checkPage=(need=14)=>{ if(y+need>276) addPage(); };
    ...
    const fmtDurPdf=m=>{ if(!m||m<=0) return 'Night'; if(m<60) return m+'m'; return Math.floor(m/60)+'h'+(m%60?'  '+m%60+'m':''); };

    /* Draw running footer (page number + tagline) on every page */
    function drawPageFooter(){
      doc.setDrawColor(200,200,200); doc.line(ML,285,RX,285);
      doc.setFontSize(7.5); doc.setFont(undefined,'normal'); doc.setTextColor(180,180,180);
    ...
    function drawHeader(){
      doc.setFillColor(42,171,153); doc.rect(0,0,PW,18,'F');
      /* subtle second stripe */
      doc.setFillColor(27,139,125); doc.rect(0,15,PW,3,'F');
    ...
    /* ── Traveller card ── */
    if(u.fullname){
      doc.setFillColor(245,252,250); doc.roundedRect(ML,y,CW,14,2.5,2.5,'F');
      doc.setDrawColor(210,240,235); doc.roundedRect(ML,y,CW,14,2.5,2.5,'S');
      doc.setFontSize(7.5); doc.setFont(undefined,'bold'); doc.setTextColor(42,171,153);
      ...
      let ci=u.email||''; if(u.phone) ci+=(ci?' · ':'')+u.phone;
      if(ci){ doc.setFont(undefined,'normal'); doc.setFontSize(8); doc.setTextColor(110,110,110); doc.text(ci,RX-4,y+10.5,{align:'right'}); }
      y+=19;
    }
    ...
    doc.setFontSize(17); doc.setFont(undefined,'bold'); doc.setTextColor(22,22,22);
    doc.text(s.dest,ML,y); y+=7;
    doc.setFontSize(9.5); doc.setFont(undefined,'normal'); doc.setTextColor(100,100,100);
    doc.text(s.days+' day'+(s.days>1?'s':'')+' · '+s.people+' traveller'+(s.people>1?'s':'')+' · '+s.style.charAt(0).toUpperCase()+s.style.slice(1)+' style · '+(s.transport==='public'?'Public transport':s.transport==='self'?'Self-drive':'Private cab')+' · '+s.pace.charAt(0).toUpperCase()+s.pace.slice(1)+' pace'+(s.date?' · Departs '+s.date:''),ML,y); y+=5;

    /* Thin teal accent line */
    doc.setDrawColor(42,171,153); doc.setLineWidth(0.6); doc.line(ML,y,ML+60,y); doc.setLineWidth(0.2); y+=8;
    ...
    bItems.forEach(([lbl,val],i)=>{
      ...
      doc.setFontSize(9.5); doc.setFont(undefined,'bold'); doc.setTextColor(42,171,153);
      doc.text(val,bx+(i>0?2:1)+1,y+11);
    });
    y+=17;

    /* Total row */
    doc.setFillColor(27,139,125); doc.roundedRect(ML,y,CW,10,2,2,'F');
    ...
    doc.text('Daily Itinerary',ML,y); y+=2;
    doc.setDrawColor(42,171,153); doc.setLineWidth(0.5); doc.line(ML,y,RX,y); doc.setLineWidth(0.2); y+=7;
    ...
    sched.forEach((day,di)=>{
      ...
      /* Day header bar */
      doc.setFillColor(232,247,245); doc.roundedRect(ML,y,CW,12,2,2,'F');
      doc.setDrawColor(180,225,218); doc.roundedRect(ML,y,CW,12,2,2,'S');

      /* DAY circle */
      doc.setFillColor(42,171,153); doc.circle(ML+7,y+6,4.5,'F');
      doc.setFontSize(5.5); doc.setFont(undefined,'bold'); doc.setTextColor(255,255,255);
      doc.text('DAY',ML+7,y+4.2,{align:'center'});
      doc.setFontSize(8); doc.text(String(day.dayNum),ML+7,y+8.2,{align:'center'});

      /* Day label and date */
      doc.setFontSize(11); doc.setFont(undefined,'bold'); doc.setTextColor(22,22,22);
      ...
      const metaParts=[];
      if(day.startTime&&day.endTime) metaParts.push(day.startTime+' – '+day.endTime);
      if(day.totalKm) metaParts.push(day.totalKm+' km');
      if(day.totalHrs) metaParts.push(day.totalHrs+' hours');
      if(metaParts.length){
      ...
      (day.activities||[]).forEach((act,ai)=>{
        ...
        /* Dot */
        doc.setFillColor(...rgb); doc.circle(DOT_X,y+3,2,'F');
        ...
        doc.setFontSize(5.5); doc.setFont(undefined,'bold'); doc.setTextColor(...rgb.map(v=>Math.max(0,v-60)));
        ...
        /* Title */
        let ty=y+6;
        doc.setFontSize(9.5); doc.setFont(undefined,'bold'); doc.setTextColor(22,22,22);
        doc.text(titleLines,ACT_X,ty);
        ty+=titleLines.length*4.5;

        /* Description */
        if(descLines.length>0){
          doc.setFontSize(7.8); doc.setFont(undefined,'normal'); doc.setTextColor(110,110,110);
    ...
    checkPage(60);
    doc.setDrawColor(200,200,200); doc.line(ML,y,RX,y); y+=7;
    doc.setFontSize(11); doc.setFont(undefined,'bold'); doc.setTextColor(30,30,30);
    doc.text('Budget Breakdown',ML,y); y+=2;
    doc.setDrawColor(42,171,153); doc.setLineWidth(0.5); doc.line(ML,y,ML+50,y); doc.setLineWidth(0.2); y+=8;
    ...
    budRows.forEach(([lbl,val,note])=>{
      ...
      doc.setFontSize(10); doc.setFont(undefined,'bold'); doc.setTextColor(42,171,153);
      doc.text(val,RX-2,y,{align:'right'});
      doc.setDrawColor(238,238,238); doc.line(ML,y+7,RX,y+7);
      y+=11;
    });
    ...
    checkPage(18);
    y+=2;
    doc.setFillColor(27,139,125); doc.roundedRect(ML,y,CW,12,2,2,'F');
    ...
    y+=16;
    if(s.people>1){
      doc.setFontSize(8.5); doc.setFont(undefined,'normal'); doc.setTextColor(110,110,110);
    ...
    checkPage(32);
    y+=4;
    doc.setFillColor(255,251,235); doc.roundedRect(ML,y,CW,28,2,2,'F');
    doc.setDrawColor(253,211,77); doc.roundedRect(ML,y,CW,28,2,2,'S');
    doc.setFontSize(8.5); doc.setFont(undefined,'bold'); doc.setTextColor(146,100,0);
    doc.text('Travel Tips for Sri Lanka',ML+4,y+6);
    doc.setFont(undefined,'normal'); doc.setFontSize(7.8); doc.setTextColor(120,80,0);
    const tips=[
...
function _legacy_renderSavedTrips(panel){
  ...
  if(!el) return;
  const trips = JSON.parse(localStorage.getItem('xtrack_trips')||'[]');
  if(!trips.length){ el.innerHTML='<div style="font-size:12px;color:var(--text3);padding:10px 4px">No saved trips yet. Save your current plan above.</div>'; return; }
...
function _legacy_loadSavedTrip(id){
  ...
  if(t.panel==='A'){ Object.assign(S.A,state); S.selA=new Set(t.sel); buildDA(); goTo('page4a'); }
  else { Object.assign(S.B,state); S.selB=new Set(t.sel); buildDB(); goTo('page4b'); }
...
function _legacy_deleteSavedTrip(id){
  ...
  localStorage.setItem('xtrack_trips',JSON.stringify(trips));
  renderSavedTrips('A'); renderSavedTrips('B');
  showToast('Deleted');
}
...
function openReview(placeName, panel){
  reviewCurPlace=placeName; reviewCurPanel=panel; reviewCurRating=0;
...
function _legacy_submitReview(){
  if(!reviewCurRating){ showToast('Please select a star rating'); return; }
...
function refreshPlaceStars(){
  $qa('[data-place-name]').forEach(el=>{
    ...
    if(starsEl && r) starsEl.innerHTML=renderStarsHTML(r.rating)+`<span class="pc-review-count">(${r.rating}.0)</span>`;
  });
}
...
function patchPlaceCards(){
  setTimeout(()=>{
    $qa('.place-card').forEach(card=>{
      ...
      if(!nameEl||card.hasAttribute('data-place-name')) return;
...
function addPriceTrends(){
  setTimeout(()=>{
    $qa('.hotel-price').forEach((el,i)=>{
      if(el.querySelector('.price-trend')) return;
...
window.switchA = function(t, el){
  _origSwitchA2(t, el);
  if(t==='overview'){ renderFXStrip('A', S.A._lastTotal); }
  if(t==='hotel'){ setTimeout(addPriceTrends,400); }
  if(t==='places'){ setTimeout(patchPlaceCards,300); }
  if(t==='export'){ renderSavedTrips('A'); }
};
const _origSwitchB2 = window.switchB;
window.switchB = function(t, el){
  _origSwitchB2(t, el);
  if(t==='overview'){ renderFXStrip('B', S.B._lastTotal); }
  if(t==='hotel'){ setTimeout(addPriceTrends,400); }
  if(t==='places'){ setTimeout(patchPlaceCards,300); }
  if(t==='export'){ renderSavedTrips('B'); }
};
...
async function generateNarrative(panel){
  ...
  try{
    ...
    if(btn){ btn.disabled=false; btn.innerHTML='<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1l1.5 3 3.3.5-2.4 2.3.6 3.2L7 8.5l-3 1.5.6-3.2L2.2 4.5 5.5 4z" stroke="white" stroke-width="1.3" stroke-linejoin="round"/></svg> Regenerate'; }
  }catch(e){
    ...
    if(btn){ btn.disabled=false; btn.innerHTML='<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1l1.5 3 3.3.5-2.4 2.3.6 3.2L7 8.5l-3 1.5.6-3.2L2.2 4.5 5.5 4z" stroke="white" stroke-width="1.3" stroke-linejoin="round"/></svg> Retry'; }
  }
}
...
async function generatePackingList(panel){
  ...
  try{
    ...
    box.innerHTML=`<div class="packing-list">${Object.entries(data).map(([cat,items]: [string, any])=>`
...
(()=>{
  setTimeout(()=>{
    $qa('.cat-card').forEach(c=>{ if(!c.getAttribute('role')){ c.setAttribute('role','button'); c.setAttribute('tabindex','0'); c.onkeydown=e=>{ if(e.key==='Enter'||e.key===' ') c.click(); }; } });
    $qa('.db-tab').forEach(t=>{ if(!t.getAttribute('role')){ t.setAttribute('role','tab'); t.setAttribute('tabindex','0'); t.onkeydown=e=>{ if(e.key==='Enter'||e.key===' ') t.click(); }; } });
    $qa('.nav-icon-btn').forEach((b,i)=>{ if(!b.getAttribute('aria-label')) b.setAttribute('aria-label', b.title||'Action '+(i+1)); });
  },500);
})();
...
async function loadReviewsFromServer() {
  ...
  try { window._serverReviews = JSON.parse(localStorage.getItem('xtrack_reviews') || '{}'); } catch(e){}
  refreshPlaceStars();
}
...
async function submitReview() {
  ...
  try {
    ...
    try {
      ...
      closeReviewModal(); refreshPlaceStars();
      showToast('⭐ Review saved for ' + reviewCurPlace);
    } catch(le) { showToast('⚠️ Could not save review.'); }
  }
}
...
async function saveProfile() {
  ...
  if (!fullname) { showToast('\u26a0\ufe0f Name cannot be empty'); return; }
  btn.disabled    = true;
...
function goToProfilePage() {
  ...
  if(el('p5BigAvatar'))    el('p5BigAvatar').textContent    = initials;
  if(el('p5HeaderName'))   el('p5HeaderName').textContent   = name  || 'My Profile';
  if(el('p5HeaderEmail'))  el('p5HeaderEmail').textContent  = email;
  if(el('p5HeaderSince'))  el('p5HeaderSince').textContent  = since;
  if(el('p5Fullname'))     el('p5Fullname').value           = name;
  if(el('p5Email'))        el('p5Email').value              = email;
  if(el('p5Phone'))        el('p5Phone').value              = phone;
  // Render trip history
  var hist = el('p5TripHistory');
  if(hist) {
    try {
      var trips = JSON.parse(localStorage.getItem('xtrack_trips') || '[]');
      if(!trips.length){ hist.innerHTML = '<div style="font-size:12px;color:var(--text3);padding:6px 0">No saved trips yet.</div>'; }
      else {
    ...
    } catch(e){ hist.innerHTML = '<div style="font-size:12px;color:var(--text3)">Could not load trips.</div>'; }
  }
  goTo('page5');
}
...
async function saveProfilePage() {
  ...
  try {
    ...
    ['userGreetingA','userGreetingB'].forEach(function(id){
      var el = $(id); if(el) el.textContent = fullname.split(' ')[0] || 'Me';
    });
    if($('p5HeaderName')) $('p5HeaderName').textContent = fullname;
...
function openAddAct(panel, dayNum) {
  _actCtx.panel = panel; _actCtx.dayNum = dayNum; _actCtx.actIdx = null; _actCtx.mode = 'add';
  const sched = S[panel].sched;
...
function openEditAct(panel, dayNum, actIdx) {
  _actCtx.panel = panel; _actCtx.dayNum = dayNum; _actCtx.actIdx = actIdx; _actCtx.mode = 'edit';
  const sched = S[panel].sched;
  const day = sched.find(d => d.dayNum === dayNum);
...
function saveAct() {
  ...
  if (!title) { $('actInpTitle').style.outline='2px solid #ef4444'; $('actInpTitle').focus(); return; }
  ...
  closeActModal();
  if (panel === 'A') renderSchA(); else renderSchB();
